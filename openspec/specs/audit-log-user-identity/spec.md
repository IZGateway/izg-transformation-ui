# Spec: Audit Log User Identity

## Purpose

Attaches the initiating user's identity and login-session context to log events
generated while handling authenticated user interactions, so events can be traced
back to the user and correlated to the originating Okta login. Identity is injected
automatically by the logging layer from a per-request context, is additive to
existing log fields, logs no secret material, and degrades gracefully when no
